...
public class App implements ActionListener, KeyListener, WindowListener { //added MouseListener
    ...
    private boolean gameOver, started, win, restart = false;
...
//        imgBG = Toolkit.getDefaultToolkit().getImage("images/flappy-bird-bg.png"); //build path folder images to source folder
        imgBG = Toolkit.getDefaultToolkit().getImage(getClass().getClassLoader().getResource("images/flappy-bird-bg.png"));
//        System.out.println("check img: " +(getClass().getClassLoader().getResourceAsStream("images/flappy-bird-bg.png")));
//        System.out.println("check img: " +ImageIO.read(getClass().getClassLoader().getResourceAsStream("images/flappy-bird-bg.png")).toString());
        ...
        //bird
        lblBird=new JLabel("");
        imgBird = Toolkit.getDefaultToolkit().getImage(getClass().getClassLoader().getResource("images/bluebird-angrybirds-3.png")); //build path folder images to source folder
        ...
        //dead bird
        imgDeadBird = Toolkit.getDefaultToolkit().getImage(getClass().getClassLoader().getResource("images/blue angry bird dead.png")); //build path folder images to source folder
        ...
        lblThreeBirds.setIcon(new ImageIcon(getClass().getClassLoader().getResource("images/theblues.gif")));
        lblThreeBirds.setBounds(310, 190, 200, 200); //x,y,width,height
        renderer.add(lblThreeBirds);

        //custom fonts
//        customFontTitle = Font.createFont(Font.TRUETYPE_FONT, new File("demo/Yackien.ttf")).deriveFont(58f);
        customFontTitle = Font.createFont(Font.TRUETYPE_FONT, getClass().getClassLoader().getResourceAsStream("fonts/Yackien.ttf")).deriveFont(58f);
/////        System.out.println("check " + getClass().getClassLoader().getResourceAsStream("fonts\\Yackien.ttf")); //mark demo_Angry_FlappyBird_intelij folder as resources root -> useable fonts folder
...
//		geTitle.registerFont(Font.createFont(Font.TRUETYPE_FONT, new File("fonts\\Yackien.ttf")));
        customFontBtn = Font.createFont(Font.TRUETYPE_FONT, getClass().getClassLoader().getResourceAsStream("fonts/GamePlayed-vYL7.ttf")).deriveFont(25f);
        customFontScore = Font.createFont(Font.TRUETYPE_FONT, getClass().getClassLoader().getResourceAsStream("fonts/GamePlayedOutline-wrX8.ttf")).deriveFont(70f);
        customFontGameOver = Font.createFont(Font.TRUETYPE_FONT, getClass().getClassLoader().getResourceAsStream("fonts/GameOfSquids-1GMVL.ttf")).deriveFont(70f);
...
//		g.fillRect(0, 0, WIDTH, HEIGHT);
        ...
        //audio
//        MP3Player mp3GTA = new MP3Player(new URL(getClass().getClassLoader().getResource("images/gta.mp3").toString()));
...
//            if(!mp3GTA.isPaused() && !mp3GTA.isStopped()) {
//                long milis = System.currentTimeMillis();
//                int nowSeconds = (int) ((milis - createdMillis) / 1000);
//            System.out.println("createdMillis: " + createdMillis);
//            System.out.println("milis: " + milis);
//            System.out.println("nowSeconds: " + nowSeconds);

//                if(nowSeconds>4) { // nowSeconds ko bắt đầu lại từ 0, mà cứ tiếp khi app đang chạy, khi click nút restart thì sẽ tiếp tục nowSeconds
//                    System.out.println("nowSeconds: " + nowSeconds);
//                    System.out.println("bef paused: " + mp3GTA.isPaused());
//                    System.out.println("bef stop:" + mp3GTA.isStopped());
////                    mp3GTA.stop();
//                    System.out.println("after paused 2: " + mp3GTA.isPaused());
//                    System.out.println("after stop 2:" + mp3GTA.isStopped());
//                }

//                mp3GTA.setRepeat(true);
//            }
//
//            if(mp3GTA.isStopped()) mp3GTA.play();
            ...
            try {
                ...
                customFontGameWin = Font.createFont(Font.TRUETYPE_FONT, getClass().getClassLoader().getResourceAsStream("fonts/PricedownBlack-MwRY.ttf")).deriveFont(60f);
    ...
    //handle start game
    public void handleStartGame() throws MalformedURLException {
        ticks++;
        ...
        if(score==100) {
            ...
            if(win) {
                try {
                    ...
                    java.util.Timer ultiTimer = new java.util.Timer();
                    TimerTask task = new TimerTask() {

                        @Override
                        public void run() {
                            if(!mp3GTA.isStopped() && !mp3GTA.isStopped()) {
                                mp3GTA.stop();
                                mp3GTA.removeAll();
                                System.exit(0);
                            }
                    ...
                    ultiTimer.schedule(task, 6600); // 6.6s
                ...
                imgMemeGlasses = Toolkit.getDefaultToolkit().getImage(getClass().getClassLoader().getResource("images/black-glass-meme.png")); //build path folder images to source folder
                ...
                lblMedal=new JLabel("");
                imgMedal = Toolkit.getDefaultToolkit().getImage(getClass().getClassLoader().getResource("images/medal.png")); //build path folder images to source folder
                ...
                imgBird = Toolkit.getDefaultToolkit().getImage(getClass().getClassLoader().getResource("images/bluebird-angrybirds-3 1.png")); //build path folder images to source folder
...
//			getRootPane().setWindowDecorationStyle(JRootPane.NONE);
            ...
            try {
                customFontLevel = Font.createFont(Font.TRUETYPE_FONT, getClass().getClassLoader().getResourceAsStream("fonts/GamePlayedOutline-wrX8.ttf")).deriveFont(30f);
            } catch (FontFormatException | IOException e) {
                e.printStackTrace();
            }
            ...
            lblClouds.setIcon(new ImageIcon(getClass().getClassLoader().getResource("images/clouds.gif")));
